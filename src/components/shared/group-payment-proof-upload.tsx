...
import { uploadGroupPaymentProof } from "@/actions/group-orders";
import { Upload, Loader2 } from "lucide-react";

export function GroupPaymentProofUpload({
  sessionCode,
  onUploaded,
}: {
  sessionCode: string;
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      startTransition(async () => {
        const updateResult = await uploadGroupPaymentProof(sessionCode, result.path);
  ...
  return (
    <div>
      ...
      <Button
        ...
        className="w-full"
        disabled={busy}
        onClick={() => document.getElementById("group-payment-proof-input")?.click()}
      >
        {busy ? (
